...
from model.PCL_TDGCN import PCL, Discriminator
from model.Adversarial import DAANLoss
import utils.utils_PCL
...
def prepare_data(args: argparse.Namespace, test_id: int) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    """
    # Load dataset
    data, label = utils.utils_PCL.load_data(args.dataset)
...
def main(test_id: int, args: argparse.Namespace) -> Tuple[float, List, List, np.ndarray]:
    """
...
if __name__ == "__main__":
    ...
    for test_id in range(15):
        ...
        best_acc, source_att, target_att, conf_matrix = main(test_id, args)
        # writer.close()
