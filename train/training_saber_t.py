...
@torch.no_grad()
def _evaluate(model: SaberT, dataloader: DataLoader, device: str, return_preds: bool = False):
    model.eval()
...
def train(
    ...
    """
    Training loop for SaberT.

    train_data / test_data shape: (sample, T, E, F)
        T = time_steps, E = num_electrodes, F = num_features
    """
    ...
    # Normalize per-sample by flattening to 2D, then restore shape
    ...
    # Keep shape as (sample, T, E, F) — SaberT expects [B, T, E, F]
    ...
    for epoch in range(epochs):
        ...
        if n_batch <= 0:
            logger.warning(
                "No valid SABER_T batch in epoch {}. Check batch_size={} with train/test sizes.",
                epoch + 1,
                batch_size,
            )
            break
        ...
        for batch_idx in range(n_batch):
            ...
            try:
                ...
                logger.exception(
                    ...
                    epoch + 1, epochs, batch_idx + 1, n_batch, subject_id, session_id,
                )
        ...
        if (epoch + 1) % 50 == 0 or epoch == 0:
            ...
            logger.info(
                ...
                epoch + 1, epochs,
                loss_clf / n_batch, loss_transfer / n_batch,
                loss_cluster / n_batch, loss_p / n_batch,
                source_acc, target_acc, best_acc,
            )
