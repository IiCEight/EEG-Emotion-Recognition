...
def _normalize_for_prpl(train_data: np.ndarray, test_data: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """Mirror TransEER PRPL normalization strategy with separate source/target scalers."""
...
@torch.no_grad()
def _evaluate(model: PRPL, dataloader: DataLoader, device: str) -> float:
    model.eval()
...
def train(
    ...
    train_data, test_data = _normalize_for_prpl(train_data, test_data)
    ...
    for epoch in range(epochs):
        ...
        if  best_acc >= (100.0 - 1e-2) or (early_stop_patience > 0 and stop >= early_stop_patience) :
