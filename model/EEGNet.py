...
class EEGNet(nn.Module):
    def __init__(self, num_electrodes=62, datapoints=128, num_classes=3, F1=8, D=2, dropout=0.5):
        ...
        #                             groups=self.F1)
        self.depth_conv = Conv2dWithConstraint(in_channels=self.F1, 
                            out_channels=self.F1 * self.D, 
                            kernel_size=(num_electrodes, 1), bias=False,
                            groups=self.F1)
    ...
    def forward(self, x):
        ...
        x = self.depth_conv(x)
        # print(f"After depth_conv : {x.shape}")
        x = self.BN2(x)
        ...
        x = self.sep_conv[0](x)
        # print(f"After sep_conv[0] : {x.shape}")
        x = self.sep_conv[1](x)
        # print(f"After sep_conv[1] : {x.shape}")
        x = self.BN3(x)
        # print(f"After BN3 : {x.shape}")
        x = self.act2(x)
        # print(f"After act2 : {x.shape}")
        x = self.pool2(x)
...
class Conv2dWithConstraint(nn.Module):
    """
    A convolutional layer with max value constraint.
    """
