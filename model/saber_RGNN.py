...
class feature_extractor(nn.Module):
    def __init__(self, num_electrodes, num_feature, layers=2, hidden_1=256, hidden_2=64, class_nums=3):
        ...
        self.adj_a = nn.Parameter(torch.tensor(get_adj_from_standard()).float(), requires_grad=True)
        self.adj_b = nn.Parameter(torch.tensor(get_adj_from_standard()).float(), requires_grad=True)

        self.MRGCN_a = MulipleResidualGCN(layers=layers, dim=1, chan_num=self.chan_num, band_num=self.band_num, hidden_1=hidden_1,
                          hidden_2=hidden_2)
        
        self.MRGCN_b = MulipleResidualGCN(layers=layers, dim=1, chan_num=self.chan_num, band_num=self.band_num, hidden_1=hidden_1,
                          hidden_2=hidden_2)
...
class feature_extractor_RGNN(nn.Module):
    def __init__(self, num_electrodes=62, in_features=5, num_classes=3, num_layers=2, num_hidden=128,
                 dropout=0.7):
        super().__init__()
