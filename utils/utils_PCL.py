...
def get_data_label_frommat(mat_path, dataset_name, session_id):
    ...
    #     mat_data = {k: v[()] for k, v in f.items()}
...
def load_data(dataset_name):
    ...
    if dataset_name in ['seed', 'seed3', 'seed4']:
        ...
        for i in range(len(allmats)):
            ...
            one_sub_data = scio.loadmat(mat_path)['XData']
            one_sub_label = scio.loadmat(mat_path)['YLabel']
...
def create_logger(args):
    ...
    console.setFormatter(logging.Formatter(datefmt='%Y-%m-%d %H:%M:%S'))
