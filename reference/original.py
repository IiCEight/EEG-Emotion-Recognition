...
from model_DANN import MSMDAERNet
from utils import load_seed4, UnalignedDataLoader,  z_score
...
def main():
    ...
    number_of_source = 15
    num_domains = 15
    path = "../EEG_DataSets/SEED_IV/eeg_feature_smooth/"
    session = 2
    ...
    for sub in range(0, 15):
        ...
        for epoch in range(max_epoch):
            ...
            for i in range(number_of_source):
                ...
                temp_loss = torch.nn.functional.log_softmax(tensor_no_diag)
                ...
                if i==14:
                    ...
                    semantic_loss, Cs_memory, Ct_memory[i] = SM(s_fea, torch.cat((features_st1, features_target1)),
                            ...
                            (torch.argmax(softmax_output15, dim=1), torch.argmax(softmax_output2, dim=1))),
                                                                ...
                                                                decay=0.9)
                    total_loss = ce_loss / 15 + alpha * (cluster_loss+0.01*semantic_loss)
                ...
                if i!=14:
                    ...
                    Cs_memory.detach_()
            model.eval()
            temp = test_suda(dset_loaders['test'], model)
        ...
        print(result_one_test[sub])

    for i, r in enumerate(result_one_test):
        print("第{}个人准确率为：{:.4f}\n".format(i + 1, r))
    print("均值为：{:.4f}".format(np.mean(result_one_test)))
    print("标准差为：{:.4f}".format(np.std(result_one_test)))
    for i, r in enumerate(result_two_test):
        print("第{}个人准确率为：{:.4f}\n".format(i + 1, r))
    print("均值为：{:.4f}".format(np.mean(result_two_test)))
    print("标准差为：{:.4f}".format(np.std(result_two_test)))
